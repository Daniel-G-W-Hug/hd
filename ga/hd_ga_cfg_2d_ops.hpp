...
template <typename T, typename U>
    requires(std::floating_point<T> && std::floating_point<U>)
inline PScalar2d_t<std::common_type_t<T, U>> wdg(const Vec2d<T>& v1, const Vec2d<U>& v2)
{
    return PScalar2d_t<std::common_type_t<T, U>>(v1.x * v2.y - v1.y * v2.x);
}
                                                             ...
                                                             const Vec2d<U>& v2)
{
    ...
    PScalar2d_t<ctype> w = wdg(v1, v2); // bivector with component e12
